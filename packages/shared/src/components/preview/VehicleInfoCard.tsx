...
export default function VehicleInfoCard(props: Readonly<VehicleInfoCardProps>) {
  ...
  const fields = [
    ...
    {
      label: t("preview.fields.transmission"),
      value: report.transmission || "—",
    },
    { label: t("preview.fields.drive"), value: report.drivetrain || "—" },
    { label: t("preview.fields.color"), value: report.color || "—" },
    {
      label: t("preview.fields.odometer"),
      value: `${report.mileage.toLocaleString()} mi`,
    },
    {
      label: t("preview.fields.title"),
      value: report.stats?.titleStatus || "—",
    },
  ];
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-3.5">
          {fields.map(({ label, value, full }) => (
            <div key={label} className={cn(full && "col-span-2")}>
              ...
              <p className="dark:text-foreground mt-0.5 text-sm font-semibold text-slate-900">
                {value}
              </p>
            </div>
          ))}
